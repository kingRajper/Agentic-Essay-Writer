    ...
    def create_interface(self):
        with gr.Blocks(theme=gr.themes.Default(spacing_size='sm',text_size="sm")) as demo:
            ...
            def updt_disp():
                ...
                # curiously, this generator returns the latest first
                for state in self.graph.get_state_history(self.thread):
                    # Skip initial states or those without complete metadata
                    if state.metadata is None or state.metadata.get('step', 0) < 1:
                        continue # Skip if metadata is missing or step is less than 1

                    # Safely access thread_ts
                    s_thread_ts = state.config.get('configurable', {}).get('thread_ts')
                    if s_thread_ts is None:
                        # If thread_ts is still not found, skip this state or handle as needed
                        # For now, let's skip it to avoid the KeyError
                        continue
                ...
                if not current_state.metadata or current_state.metadata.get('step', 0) < 1: #handle init call and empty metadata
                    # Return sensible defaults if the state is not fully initialized
                    return {
                        topic_bx : "", # Or a default topic if appropriate
                        lnode_bx : "",
                        count_bx : 0,
                        revision_bx : 0,
                        nnode_bx : [], # next can be empty list
